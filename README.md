...
[**Download `europe.dta.zip` (1.37 GB)**](https://github.com/blanc-g/crowdsourced/releases/download/v1.0/europe.dta.zip)

The release details and SHA-256 checksum are available on the [v1.0 release page](https://github.com/blanc-g/crowdsourced/releases/tag/v1.0).
...
The release contains an individual-level dataset derived from the public FamiLinx genealogies assembled by Kaplanis et al. (2018) from public Geni.com profiles. The unit of observation is an anonymized individual profile. This is a crowdsourced genealogy sample rather than a random sample of the historical population. The accompanying paper evaluates its representativeness.

`europe.dta` contains 9,426,965 observations and 119 variables, in Stata format 118 (Stata 14 or later). It covers individuals born and died between 1400 and 2018.

## Geographic Coverage and Sample Selection

Selection is based on three events: the individual's birth, the birth of the individual's first linked child, and the individual's death.

Coverage extends to Austria, Belgium, Bulgaria, Croatia, Czechia, Denmark, Estonia, Finland, France, Germany, Greece, Hungary, Iceland, Ireland, Italy, Latvia, Lithuania, Luxembourg, North Macedonia, the Netherlands, Norway, Poland, Portugal, Romania, Slovakia, Slovenia, Spain, Sweden, Switzerland, and the United Kingdom.

An individual is included in `europe.dta` when at least one selection event maps to one of these countries. Birth in Europe is not required. For example, an individual born outside Europe who had a first linked child in Europe or died in Europe is included. After inclusion, all retained event fields are preserved, including fields for events outside Europe.

In the string country variables, North Macedonia and the United Kingdom are recorded as `Macedonia` and `UK`.

Non-European life events are not discarded for people selected into `europe.dta`. When such an event maps to the United States or Canada, its coordinates and place fields remain in the observation. `america` and the event-specific `*_america` variables describe those retained event locations; they do not define an additional North American sample.

## Data Source and Construction

The FamiLinx source is reported to contain 86,124,645 individual profiles plus direct parent-child links. The released dataset was constructed in four stages.

### Profiles, dates, and plausibility screens

- Profiles are imported in 2,000,000-row blocks.
...
First- and last-child years, monthly dates, months, and locations are derived from linked children whose profiles could be matched. Relation counts can therefore be observed even when dated child profiles are not.

The fertility-sample indicator is constructed from the individual's own linked fertility and a construction-only ancestral measure, `fert_bef`. The latter is the row mean of the nonmissing linked-child counts in 30 pedigree positions: 2 parents, 4 grandparents, 8 great-grandparents, and 16 great-great-grandparents. Missing positions are ignored, and `fert_bef` is missing only when fertility is unavailable for all 30 positions. Pedigree collapse can cause the same ancestor to occupy more than one position.

The indicator is defined only when both the individual's `fert` and `fert_bef` are observed:

- `s_fert` is missing when the individual's own `fert` is missing or all 30 ancestral fertility values are missing.
- `s_fert==0` when own and ancestral fertility are observed but `fert_bef<=1`.
- `s_fert==1` when own and ancestral fertility are observed and `fert_bef>1`; this is the analytic fertility sample.

Every nonmissing linked-child count is an integer of at least one. Consequently, `fert_bef>1` is equivalent to at least one observed pedigree position having more than one linked child. Likewise, `s_fert==0` means that every observed pedigree-position count equals one. Missing `fert` or `s_fert` must not be interpreted as zero fertility: a profile without a retained linked child is assigned missing `fert`, not `fert=0`.

Observed fertility coverage in `europe.dta`: of the 9,426,965 total observations, `fert` is observed for 2,070,330. Among those, `s_fert` is missing for 8,313,251 (ancestral fertility unavailable), `s_fert==0` for 353,890, and `s_fert==1` for 759,824 — the analytic fertility sample. `s_fert` is nonmissing for 1,113,714 observations overall.

Among profiles with observed own fertility, 956,616 have missing `s_fert` because ancestral fertility is unavailable. The file retains main-sample profiles regardless of whether `s_fert` is observed, which is why its total observation count is much larger than its fertility-sample count.

### Geographic mapping and historical urban status

European coordinate keys are matched using 2018 LAU geography and 2013 NUTS 3, NUTS 2, and country-level geography. For retained events that map to the United States or Canada (see above), coordinate keys are matched to 2019 Census counties and states, or 2016 Canadian census divisions and provinces, respectively. These modern units are applied to historical event coordinates. The released `*_dep`, `*_region`, and `*_country` values are dataset-specific integer categories with Stata value labels generated from area names; they are not official geographic codes. For purposes of matching historical city populations, selected region-level consolidations are applied for London, Brussels, Antwerp, Liège, Dublin, and Belfast.
...
### Derived variables, parents, and weights
...
- `america`: auxiliary indicator equal to one when at least one retained event maps to the United States or Canada; it does not determine inclusion in this file.
- `s_main`: main-sample indicator. It is one for every observation in this file.
- `s_fert`: fertility-sample indicator. It is missing unless both own and ancestral fertility are observed; `s_fert==1` identifies the analytic fertility sample. Counts are reported in the construction section above.
...
The fertility sample retains an individual when own fertility is observed, at least one of the 30 preceding-generation pedigree positions has observed fertility, and at least one observed position has more than one linked child. Childless individuals cannot be identified reliably. The measure is completed gross linked fertility; child mortality is under-recorded outside direct vertical lineages.
...
- Year, monthly date, place identifier, raw coordinate key, country, and Europe indicator for birth, first linked child's birth, and death. These profile characteristics require the parent to be present in the retained intermediate individual sample. The released parent-event fields include Europe indicators only; corresponding parent-event America indicators are not included.
...
- `*_id`: mapped LAU identifier in Europe or source coordinate-key identifier in the United States/Canada.
- `*_dep`: dataset-specific integer category with value labels for NUTS 3 names in Europe, county names in the United States, or census-division names in Canada; not an official code.
- `*_region`: dataset-specific integer category with value labels for NUTS 2 names in Europe, state names in the United States, or province names in Canada; not an official code.
...
- `*_latitude`, `*_longitude`: coordinates of the mapped place. European values are LAU representative points; United States and Canadian values retain the matched source-coordinate points.
- `*_europe`, `*_america`: event-area indicators. Here, `*_america` refers specifically to the United States or Canada.

No other last-child geography fields are included.

| Event-level fields | Event in Europe | Event in United States or Canada |
|---|---|---|
| `*_id`, `*_dep`, `*_region`, `*_country`, `*_COUNTRY`, coordinates | Available when mapped | Available when mapped |
| `*_group` | Available | Missing |
| `*_urban` | Available where historical urban data are covered | Missing |
| `*_weight` | Available for mapped European countries | Missing |

### Urban status, migration, and weights

- `birth_urban`, `first_child_urban`, and `death_urban` indicate that the corresponding mapped European municipality had at least 5,000 inhabitants at the latest available Bairoch benchmark at or before the individual's year of birth, where historical urban data are covered. `death_urban` therefore describes the death place's urban status at the individual's birth benchmark, not at the year of death.
...
- `birth_weight`, `first_child_weight`, and `death_weight` are reweighting factors for mapped European event countries. The numerator is the country's estimated population in 1820, measured in thousands. The denominator is the number of genealogical observations for that event and country in the corresponding half-open main window: `[1675,1900)` for birth, `[1700,1925)` for first child, and `[1750,1975)` for death. They are missing for United States and Canadian events and are not normalized to a mean of one.

Use the weight corresponding to the event-time outcome being analyzed.

## Recommended Analysis Windows

For its European analyses, the paper focuses on periods where comparisons with representative sources suggest selection is most limited:
...
The file retains observations outside these windows.

## Variable Dictionary

The dataset has the following 119-variable schema.
...
| 6 | `america` | Auxiliary indicator equal to 1 if any retained birth, first-child, or death location maps to the United States or Canada; missing otherwise. It does not determine inclusion in this file. |
| 7 | `fert` | Number of retained parent-child link rows; use `s_fert==1` for fertility analysis. |
| 8 | `s_fert` | Fertility-sample indicator: missing if own fertility or all ancestral fertility values are missing; 0 if both are observed and every observed ancestral linked-child count equals 1; 1 if both are observed and at least one observed ancestral count exceeds 1. |
| 9 | `s_main` | Main-sample indicator; equals 1 for every observation in both released files. |
...
| 64 | `birth_id` | 2018 LAU identifier in Europe or source coordinate-key identifier in the United States/Canada for the birth place. |
...
| 71 | `birth_america` | Birth-location indicator: 1=United States/Canada, 0=Europe, missing if unmapped. |
| 72 | `birth_longitude` | Longitude of mapped birth place; a representative LAU point in Europe or matched source-coordinate point in the United States/Canada. |
| 73 | `birth_latitude` | Latitude of mapped birth place; a representative LAU point in Europe or matched source-coordinate point in the United States/Canada. |
...
| 81 | `first_child_id` | 2018 LAU identifier in Europe or source coordinate-key identifier in the United States/Canada for the mapped first-child place; a fallback may use the earliest dated child with a location. |
...
| 88 | `first_child_america` | First-child-location indicator: 1=United States/Canada, 0=Europe, missing if unmapped. |
| 89 | `first_child_longitude` | Longitude of mapped first-child place; a representative LAU point in Europe or matched source-coordinate point in the United States/Canada. |
| 90 | `first_child_latitude` | Latitude of mapped first-child place; a representative LAU point in Europe or matched source-coordinate point in the United States/Canada. |
...
| 101 | `death_id` | 2018 LAU identifier in Europe or source coordinate-key identifier in the United States/Canada for the death place. |
...
| 108 | `death_america` | Death-location indicator: 1=United States/Canada, 0=Europe, missing if unmapped. |
| 109 | `death_longitude` | Longitude of mapped death place; a representative LAU point in Europe or matched source-coordinate point in the United States/Canada. |
| 110 | `death_latitude` | Latitude of mapped death place; a representative LAU point in Europe or matched source-coordinate point in the United States/Canada. |
...
- Guillaume Blanc (2024), “Demographic transitions, rural flight, and intergenerational persistence: evidence from crowdsourced genealogies,” mimeo, data available at [crowdsourced.guillaumeblanc.com](https://crowdsourced.guillaumeblanc.com/).
